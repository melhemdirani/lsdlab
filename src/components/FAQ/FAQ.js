...
const Question = ({question, text, delay}) => {
    ...
    return(
        <div className='Question_Container' onClick={handleClick} >
            <div className='row1'>
                <p>{question}</p>
                <p className='button'>{showText ? "-" : "+"}</p>
            </div>
            {
...
function FAQ() {
    ...
    let answer3 = ["TBA"]
    let answer4 = ["Download the MetaMask extension for Google Chrome.", "Buy ETH on an exchange like Binance, OKEx, Huobietc, etc.. Then send it to your MetaMask Wallet.", `Connect your Metamask Wallet to the website by clicking "WalletConnect."`, `Finally, click "Mint Now" on our website and approve the transaction on Metamask.`]
